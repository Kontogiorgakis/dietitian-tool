...
- **Prisma 7 does not read `.env` itself.** `prisma.config.ts` loads `.env` and `.env.local` with `process.loadEnvFile`; a "placeholder:5432" error means that loading broke, not that the file is missing. `DIRECT_URL` alone is enough in development.
- **The direct Supabase host (`db.<ref>.supabase.co`) is IPv6-only.** On this network it does not connect. Use the session pooler string (`postgres.<ref>@aws-1-eu-west-1.pooler.supabase.com:5432`); the node is `aws-1`, not `aws-0`, and a "tenant not found" reply means the wrong node.
- **Puppeteer `clip` and `fullPage` captures lie about Recharts.** Both re-lay-out the page during capture, so charts come out tiny or with giant text while the DOM is correct. Capture the plain viewport (tall if needed) and crop afterwards with sharp; when a chart looks wrong, measure the SVG's `viewBox` against its rect before touching code.
- **Puppeteer hovers right after `networkidle2` are lost to hydration**, and a transition caught mid-fade samples the wrong color. Wait about a second before hovering, and inject `* { transition: none !important }` when sampling pixels.
- **Killing `next dev` can leave `.next` unreadable.** Every route then 404s and the log says "Unexpected end of JSON input" while the message files validate fine. Delete `.next` and start again.
- **tailwind-merge must know custom `text-*` sizes.** `cn()` in `lib/general/utils.ts` extends the `font-size` group with the Μέτρο scale; without it `text-num-xl` is read as a color and silently dropped by `text-accent`. Add any new type-scale name there too.
